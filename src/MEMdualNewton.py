...
from numpy import any, logical_and, all
...
#@jit
def MEMdN_solve(A, b, C, mu,  alpha_min=1e0, alpha_max=1e8, Nalpha = 19, posterior_cutoff=0.1, \
                rtol=1e-8, atol =1e-8, numerical_zero=1e-12):
    # 1) This code optimizes ||A@x-b||^2_C + \alpha S_sj
    # in the interval [alpha_min, alpha_max] using dual Newton optimization algorithm.
    # "Dual formulation of the MEM applied to AC of QMC data"
    ...
    # 2) Then this function combines the solutions into a single estimate by weighting combination
    ...
    #   accepted_arr: elements are 0 or 1 indicating
    #                 whether the corresponding alpha was included in the averaging.
    ...
    Cinv = inv(C)
    model = DPModel(A, b[:,0], q=mu[:,0], C=C)
    # Optionally: DPModel(A, b, q=None, C=None, c=None, λ=None)
    ...
    for i in range( Nalpha ):
        ...
        x[:,0] = solve(model, atol=atol, rtol=rtol) # solve for alphas in domain
